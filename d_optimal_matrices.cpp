...
bool verify_solution(std::vector<quadratic_eq>& quadratics, std::vector<std::vector<std::vector<int> > >& sequences_indices, std::vector<std::vector<char> >& sequences, int& qua_target){
	...
	for(auto qua: quadratics){
		if(qua.pos_one_count-qua.neg_one_count!=qua_target){
			return false;
		}
	}
	return true;
...
void quadratics_check(std::vector<std::vector<char> >& sequences, std::vector<quadratic_eq>& quadratics, int& pos_index, int& neg_index,  std::vector<std::vector<int> >& qua_temp, int& seq, int& score_temp, int& n, int& qua_target){
	...
	for(auto qua: quadratics){
		...
		if(pos_index != (neg_index-step+n)%n){
			...
			if(sequences[seq][(pos_index+step)%n]==1){
			...
			if(sequences[seq][(neg_index-step+n)%n]==-1){
				if(pos_neg_count[0]-pos_neg_count[1]>qua_target){
                    score_temp=score_temp-2;
                }
                else{
		...
		if((pos_index-step+n)%n != neg_index){
			...
			if(sequences[seq][(pos_index-step+n)%n]==1){
				if(pos_neg_count[0]-pos_neg_count[1]>qua_target){
                    score_temp=score_temp-2;
                }
                else{
			...
			if(sequences[seq][(neg_index+step)%n]==-1){
				if(pos_neg_count[0]-pos_neg_count[1]>qua_target){
                    score_temp=score_temp-2;
                }
                else{
...
void local_search(std::vector<std::vector<char> >& sequences, std::vector<std::vector<std::vector<int> > >& sequences_indices, std::vector<quadratic_eq>& quadratics, int& n, int& seq_count, int& quadratic_count, int& qua_target, int& wp){
	...
	int seq_rand;
	int wp_rand;
	std::vector<min_neighbour> min_neighbour_list;
	...
	/*main loop*/
	int iteration=0;
	while(!verify_solution(quadratics, sequences_indices, sequences, qua_target)){
		++iteration;
		
		wp_rand = rand()%100+1; //random from 1-100
		/*probability wp*/
		if(wp_rand<=wp){
		...
		/*probability 1-wp*/
			...
			min_neighbour_list.clear();
			score_min=score;
			score_temp=score;
			std::cout<<"current score is: "<<score<<std::endl;
			std::cout<<"current iteration is: "<<iteration<<std::endl;
				...
				for(auto pos_index: sequences_indices[seq][0]){
					for(auto neg_index: sequences_indices[seq][1]){
						...
						if(abs(score_temp)<abs(score_min)){
							min_neighbour_list.clear();
							min_neighbour_list.emplace_back(min_neighbour());
							min_neighbour_list[min_neighbour_list.size()-1].seq_min=seq;
							min_neighbour_list[min_neighbour_list.size()-1].pos_min=pos_index;
							min_neighbour_list[min_neighbour_list.size()-1].neg_min=neg_index;
							min_neighbour_list[min_neighbour_list.size()-1].qua_min=qua_temp;
							score_min=score_temp;
							//std::cout<<"score_min is: "<<score_min<<std::endl;
						}
						//if score is same as current score_min, then push this neighbour to a list.
						else if(abs(score_temp)==abs(score_min)){
							min_neighbour_list.emplace_back(min_neighbour());
							min_neighbour_list[min_neighbour_list.size()-1].seq_min=seq;
							min_neighbour_list[min_neighbour_list.size()-1].pos_min=pos_index;
							min_neighbour_list[min_neighbour_list.size()-1].neg_min=neg_index;
			...
			//if no better configuration found, swap a random pair of +1 and -1
			if(score<=score_min){
				std::vector<std::vector<int> > qua_temp;
				score_temp=score;
				//randomly pick a sequence
				...
				//pick one neighbour from min_neighbour_list randomly
				int randneighbour=rand()%min_neighbour_list.size();
				seq_min=min_neighbour_list[randneighbour].seq_min;
				pos_min=min_neighbour_list[randneighbour].pos_min;
				neg_min=min_neighbour_list[randneighbour].neg_min;
				qua_min=min_neighbour_list[randneighbour].qua_min;
			}
		}
	    score=score_min;
		std::cout<<"score_min is "<<score_min<<std::endl;
...
int main(int argc, char *argv[]){
	int n=atoi(argv[1]);
	int wp=atoi(argv[4]); //noise setting
	...
	*/
	...
	/*local search*/
	local_search(sequences, sequences_indices, quadratics, n, seq_count, quadratic_count, qua_target, wp);	
